"""get_business_children 工具：获取指定业务节点的子节点导航信息。"""
...
async def get_business_children(
    ctx: RunContext[AgentDeps],
    node_id: Annotated[str, Field(description="业务节点 ID，首次调用传 'root'")] = "root",
) -> str:
    """获取指定业务节点的子节点列表（id、name、keywords），用于导航决策。首次调用传 node_id='root' 从根节点开始。"""
    sid: str = ctx.deps.session_id
    rid: str = ctx.deps.request_id
    params: dict[str, object] = {"node_id": node_id}
    log_tool_start("get_business_children", sid, rid, params)

    try:
        service: BusinessMapService = _get_service()
        result: str = service.get_business_children_nav(node_id)
        log_tool_end("get_business_children", sid, rid, {"node_id": node_id, "result_len": len(result)})
        return result
    except KeyError:
        error_msg: str = f"节点 '{node_id}' 不存在。请检查 node_id 是否正确。"
        log_tool_end("get_business_children", sid, rid, {"error": error_msg})
        return error_msg
