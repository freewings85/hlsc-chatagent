...
根据已确认的省钱目标、项目类型和消费偏好，从 6 种方法中选择最优组合。

## 偏好使用规则

- 优先使用用户画像中的消费偏好；没有时，仅在方法选择明显受影响时顺手确认。
- A/B 档可更积极地使用比价、换商户、换配件档次等方法。
- C 档适合给 2 档平衡方案供车主选择。
- D/E 档少推荐折腾成本高、等待时间长或不确定性高的方法，更多保留省心、省时方案。
- 紧急/救援类场景中，优先快速可执行的方法，消费偏好只作为次要修正。
...
| B. 切换商户 | 找更低价类型商户，或同类多商户报价对比 | 需配合商户选择 |
