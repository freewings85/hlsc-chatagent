...
[待收集信息]
  ...
  养车项目名称: (待收集) — 从匹配结果获取
  车型信息: 2021款大众朗逸 — 已有
[可用工具] match_project, collect_car_info
...
scenes:
  ...
  LIFECYCLE_PROJECT:
    ...
    target_slots:
      ...
      vehicle_info:
        label: 车型信息
        required: true
        method: 调用 collect_car_info 获取精确车型
    tools: [match_project, collect_car_info, knowledge_base_search]
    exit_when: "slot.project_id"
    strategy: |
      结合里程和上次保养时间推荐应做项目。
      需要车型信息来精确推荐。
  ...
  FIND_MERCHANT:
    ...
    tools: [merchant-search-and-confirm, search_shops, collect_location]
