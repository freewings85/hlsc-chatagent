"""优惠活动搜索条件的 typed schema。

扁平 schema：LEAF.params 合并所有字段，不分活动维度/商户维度，后端按字段名自拆。

schema 只作为 submit_coupon_search_criteria 工具的参数类型出现——LLM 在 tool-call
...
from hlsc.models.shop_search import SortBy


class CouponSearchLeafParams(BaseModel):
    """LEAF.params 的可用字段（扁平，不分活动/商户维度）。
    ...
    radius: int | None = Field(
        ...
        description=(
            "搜索半径，单位米（公里数 × 1000）。"
            "只有用户明说距离数字才填。'附近'不算明确距离。"
        ),
    )
    project_keywords: list[str] | None = Field(
        default=None,
        description="用户原话里对服务项目的描述，原样传。用户没说不填。",
    )
    ...
    fuzzy_keywords: list[str] | None = Field(
        default=None,
        description=(
            "用户原话里对商户的其他要求描述（不放位置/商户/项目名之类已有字段能接的词），"
            "原样传。用户没说不填。"
        ),
    )
    activity_type_text: str | None = Field(
        default=None,
        description="用户原话里对优惠形态的归纳分类词，原样传。用户没说不填。",
    )
    activity_keywords: list[str] | None = Field(
        default=None,
        description=(
            "用户原话里对活动本身的自由描述词（不是优惠形态分类本身），"
            "原样传。用户没说不填。"
        ),
    )
    brand_names: list[str] | None = Field(
        default=None,
        description=(
            "用户原话里提到的品牌名，只填品牌本名"
            "（不要带'品牌'、'4S'之类后缀）。用户没说不填。"
        ),
    )
...
class CouponSearchQuery(BaseModel):
    ...
    params: CouponSearchLeafParams | None = Field(
        default=None,
        description="仅 op=LEAF 时使用。",
    )
    children: list["CouponSearchQuery"] | None = Field(
        default=None,
        description="仅 op=AND 或 op=OR 时使用。",
    )


class CouponSearchInfo(BaseModel):
    """找优惠活动条件整包。query 必填。"""
