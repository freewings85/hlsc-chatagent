...
# _DESCRIPTION = load_tool_prompt("get_project_price")
...
#         summary = _build_summary(result, distance_km, min_rating, sort_by)
...
# def _build_summary(result, distance_km, min_rating, sort_by) -> str:
#     shops = result.shops
#     filter_desc = f"{distance_km}km 范围内"
#     if min_rating:
#         filter_desc += f"、评分 {min_rating} 以上"
#     sort_desc = {"distance": "按距离", "rating": "按评分", "price": "按价格"}.get(sort_by, "")

#     lines = [f"在{filter_desc}找到 {len(shops)} 家门店的报价（{sort_desc}排序）：", ""]

#     for shop in shops:
#         rating_text = f", 评分{shop.rating}" if shop.rating else ""
...
#                     type_label = REPAIR_TYPE_NAMES.get(plan.type, plan.type)
#                     qa_text = f", 保质期{plan.qa}" if plan.qa else ""
