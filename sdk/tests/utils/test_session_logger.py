...
def log_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """使用临时目录作为日志目录，避免污染项目"""
    monkeypatch.setattr("agent_sdk._utils.session_logger._LOG_DIR", str(tmp_path))
    # 清除 logger 缓存，避免复用旧 handler
    import agent_sdk._utils.session_logger as sl
...
class TestLogToolStartEnd:
    def test_tool_start_end(self, log_dir: Path) -> None:
        from agent_sdk._utils.session_logger import log_tool_end, log_tool_start
    ...
    def test_tool_with_exception(self, log_dir: Path) -> None:
        from agent_sdk._utils.session_logger import log_tool_end
    ...
    def test_no_context_silent(self, log_dir: Path) -> None:
        """没有设置上下文时不报错"""
        from agent_sdk._utils.session_logger import log_tool_start
        log_tool_start("read")  # 不应抛出


class TestLogLlmStartEnd:
    def test_llm_text_response(self, log_dir: Path) -> None:
        from agent_sdk._utils.session_logger import log_llm_end, log_llm_start
    ...
    def test_llm_error(self, log_dir: Path) -> None:
        from agent_sdk._utils.session_logger import log_llm_end
    ...
    def test_llm_start_with_messages_detail(self, log_dir: Path) -> None:
        ...
        from agent_sdk._utils.session_logger import log_llm_start
...
class TestLogHttpRequestResponse:
    def test_http_success(self, log_dir: Path) -> None:
        from agent_sdk._utils.session_logger import log_http_request, log_http_response
    ...
    def test_http_error(self, log_dir: Path) -> None:
        from agent_sdk._utils.session_logger import log_http_response
...
class TestLogRequestStartEnd:
    def test_request_lifecycle(self, log_dir: Path) -> None:
        from agent_sdk._utils.session_logger import log_request_end, log_request_start
...
class TestLogInfoDebugError:
    ...
    def test_log_debug(self, log_dir: Path) -> None:
        from agent_sdk._utils.session_logger import log_debug
    ...
    def test_log_error_with_exc(self, log_dir: Path) -> None:
        from agent_sdk._utils.session_logger import log_error
