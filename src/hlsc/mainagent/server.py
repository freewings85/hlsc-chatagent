...
启动方式：
    ...
    uv run python src/hlsc/mainagent/server.py --env src/hlsc/mainagent/.env.local
    uv run python src/hlsc/mainagent/server.py --env src/hlsc/mainagent/.env.uat --port 8100
"""
...
def _load_env() -> argparse.Namespace:
    """解析 --env 参数并加载配置（必须在任何 src.sdk import 之前）。"""
...
def main() -> None:
    ...
    from src.sdk._common.nacos import register_service, deregister_service
    ...
    from src.sdk._config.settings import LogfireConfig
    ...
    # 创建 Agent + AgentApp
    from src.hlsc.mainagent.app import create_main_agent

    from src.sdk import AgentApp, AgentAppConfig
